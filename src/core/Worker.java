...
public class Worker {
    ...
    public int bouge() // SEULE FONCTION A COMPLETER !
    {
        ...
        Direction objectif = null;

        terrain.vague_new_cell(currentPos);
        ...
        if (etat == Etat.RECHERCHE) {
            if (terrain.isFoodAtPoint(currentPos)){
                ...
                //cherche dans le voisinage proche de la nouriture
                objectif = searchForFood();
                // si il n'en trouve pas rechercher par les pheromones
                if (objectif == null){
            ...
            if (terrain.isColonyAtPoint(currentPos)){
                ...
                dropFoodPheromone();
                /*
                objectif = direction(colony.getPosition());*/
                objectif = direction(terrain.goHome(currentPos));
            }
        }
        // gère évitement d'obstacles
        newDirection = decideNewDirectionForDirection(objectif);
