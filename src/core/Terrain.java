...
public class Terrain extends JPanel {
    ...
    Terrain(JFrame newParent) {
        ...
        vague = new int[panelWidth][panelHeight];
        for (int i = 0; i < panelWidth; i++){
            for (int j = 0; j < panelHeight; j++){
                vague[i][j] = Integer.MAX_VALUE;
            }
        }
        vague[base.pos.x][base.pos.y] = -1;

    }

    public void vague_new_cell(Position position){
        int x,y;
        x = position.x;
        y = position.y;
        int value = 0;
        if (x == 0){
            if (y == 0){
                value = Math.min(vague[x+1][y], vague[x][y+1]) +1;
            }else if(y == panelHeight-1){
                value = Math.min(vague[x+1][y], vague[x][y-1]) +1;
            }else {
                value = Math.min(vague[x+1][y], Math.min(vague[x][y-1], vague[x][y+1]))+1;
            }
            ...
            if (y == 0){
                value = Math.min(Math.min(vague[x-1][y], vague[x+1][y]), vague[x][y+1]) +1;
            }else if(y == panelHeight-1){
                value = Math.min(Math.min(vague[x-1][y], vague[x+1][y]), vague[x][y-1]) +1;
            }else {
                value = Math.min(Math.min(vague[x - 1][y], vague[x + 1][y]), Math.min(vague[x][y - 1], vague[x][y + 1])) + 1;
            }
        }
        vague[x][y] = Math.min(value, vague[x][y]);
    }

    public Position goHome(Position position){
        int x, y;
        x = position.x;
        y = position.y;
        ...
        if (x > 0 && vague[x-1][y] < bestValue){
            bestValue = vague[x-1][y];
            retPos = new Position(x-1, y);
        }
        if ( x < panelWidth-1 && vague[x+1][y] < bestValue){
            bestValue = vague[x+1][y];
            retPos = new Position(x+1, y);
        }
            ...
            for (int i = 0; line != null && i < line.length() && i < panelWidth; i++) {
                ...
                switch (car) {
                    ...
                    case 'X':
                        ...
                        break; //OBSTACLE
                    case 'F':
                        food[i][j] = INIT_FOOD_QUANTITY_PER_CELL;
                        fileFoodQuantity++;
                        break; // FOOD
                    case 'B':
                        base.setPosition(new Position(i, j));
